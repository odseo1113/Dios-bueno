...
def guardar_respuesta(tipo, palabra, respuesta):
    ...
    cursor.execute("""
        SELECT id
        FROM respuestas
        WHERE TRIM(LOWER(tipo)) = TRIM(LOWER(%s))
        AND TRIM(LOWER(palabra)) = TRIM(LOWER(%s))
        LIMIT 1
    """, (
        tipo,
        palabra
...
def guardar_cita(cliente, negocio, fecha):
    ...
    cliente = normalizar_numero(cliente)
    negocio = normalizar_numero(negocio)
